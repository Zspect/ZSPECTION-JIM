...
import {Platform,StyleSheet,View,StatusBar,ScrollView,Image,TouchableOpacity,FlatList,BackHandler,TextInput} from 'react-native';
import styles from '../../assets/styles/style2.js';
import styles2 from '../../assets/styles/style.js';
import {Container,Header,Content,Card,CardItem,Right,Left,Switch,Text,Body,Title,Form,Item,Toast} from 'native-base';
...
export default class RegisterPriceMatrix extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      priceMatrix: [],
      foundationList: [],
      categoryList: [...this.props.navigation.state.params.selectedCategory],
      pageList: [],
      UICArray: [],
      selectedPrice:0,
      selectedDuration:0,
  ...
  async getFoundation() {
    for (let index=0;index<this.state.categoryList.length;index++){
         await this.fetchRecord(this.state.categoryList[index])
    }
  }
  fetchRecord = async element => {
    this.setState({loading: true});
    let response = await new API('PriceMatrix', {}).getApiResponse('/' + element.id);
    console.log("dsabdjsabsadsad >>",response)
  ...
  saveMatrix = async () => {
    ...
    await this.getRequestData().then(data => {
      ...
      let response = new API('CompanyPrices', data).getResponse();
      console.log('Company matrix response: ', response);
      ...
      <ScrollView
        ...
        <Header style={{backgroundColor: '#28558E'}}>
          <Body style={{justifyContent: 'center', alignItems: 'center'}}>
            <Title>Company Price Matrix</Title>
            {this.getTitle()}
          </Body>
        </Header>
